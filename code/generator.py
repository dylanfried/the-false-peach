...
class Generator:
   ...
   #Get current filter takes a trail.
   # -trail: Is the data structure described in the comments for __init__
   ...
   #   markov.py.
   def getCurrEmoFilter(self, trial, word_index, pos_word_key):
   ...
   def getCurrOrder(word_index, pos_word_key):
      if pos_word_key == "pos":
         order_ramp = trial["POS_order_ramp"]
      else:
         order_ramp = trial["word_order_ramp"]
      for i in len(order_ramp):
         if POS_order_ramp[i]["word_number"]>word_index:
            break
         current_pos_order = POS_order_ramp[i]["order"]
   ...
   #      the next word.
      for trial in range(len(trials)): 
         ...
         word_markov = Markov(trial["POS_training_text"], max_word_order, 11)
         word_markov.initialize()
         
         #initialize the current order to the first order.
         current_pos_order = trail["POS_order_ramp"][0]["order"]
         current_word_order = trail["POS_order_ramp"][0]["order"]
         
         #initialize the word and pos filters.
         current_pos_filter = getCurrEmoFilter(trial, 1, "pos")
         current_word_filter = getCurrEmoFilter(trial, 1, "word")
         for i in range(trial["trial_length"]):
            current_pos_order = getCurrOrder(i, "pos")
            current_pos_filters = getCurrEmoFilter(trial, i, "pos")
            current_pos = pos_markov.generateNext(current_pos_order, current_pos_filters)
            
            current_word_order = getCurrOrder(i, "word")
            current_pos_filters = getCurrEmoFilter(trial, i, "word")
            current_pos_filters.append({"index": 10, "filter": current_pos, "type": "text_match"})
            current_word = pos_markov.generateNext(current_pos_order, filters)
