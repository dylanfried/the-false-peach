   ...
   class voice_source(datum):
      ...
      def __init__(self,ip,port):
         ...
         def aff(addr,tags,stuff,source):
            ...
            global threshold
            global next_line
            affect = min(stuff[0],3)
            affect = int(max(-1,affect))+5
            next_line[0] = ""
         ...
         if ctype == "livethreshold":
            ...
            while lk > 0 and not newmove:
               ...
               if indie in contexts[lk-1][nmax-1].keys():
                  ...
                  tc = [t for t in trial_data if t[affect]>threshold or t[nmax-1]=="NEWLINE"]
            ...
            if out and (newmove[nmax-1] == "NEWLINE"):

               l = clean(out)

               if re.match("^[A-Z_]+$",l.strip()): 
