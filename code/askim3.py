   ...
   if s.name == "sp":
      ...
      for l in s.findAll(["l","ab"]):
         ...
         for w in l.findAll("w"):
            ...
            wp = w["pos"].encode('utf-8')

            if ww: 

               if outl: outl = outl + " " + ww
               else: outl = ww

               if outp: outp = outp + " " + wp
               else: outp = wp
         ...
         if not len(outl) == len(outp): print "OH NO!!"

         for ii in range(len(outl)):
      ...
      for w in l.split(" "):
         ...
         ofile.write(act+" "+scene+" "+lineno+" -1 "+"Stage -1 -1 -1 -1 -1 Stage "+w+"\n")
